...
fn list_databases_command(application: &String, zone: &String, project: &String) {
    ...
    let database_instance =
        gcloud::list(Application::new(&application), &Zone::new(&zone), project);
...
fn parse_gcloud_error(error_message: &str) -> Result<(String, String), &'static str> {
    ...
    let permissions_regex =
        Regex::new(r" - Required '(.+)' permission for 'projects/(.+)'").unwrap();
    let auth_error_pattern = Regex::new(
        r"You do not currently have an active account selected.|Please run:\s+\$ gcloud auth login",
    )
    .unwrap();
    let expired_auth_regex =
        Regex::new(r" - Request had invalid authentication credentials").unwrap();
...
fn connect_to_database_command(
    application: &String,
    port: &Option<u16>,
    instance: &Option<String>,
    zone: &String,
    ...
    let database_instance = match instance {
        ...
        None => {
            let mut database_instance =
                gcloud::list(Application::new(&application), &Zone::new(zone), project).unwrap();
            gcloud::connect(
                port,
                &database_instance.pop().unwrap(),
                Zone::new(zone),
                project,
            )
            .unwrap();
...
fn connect_to_db_interactive() {
    ...
    log::debug!(
        "Connecting to DB with data: Zone: {} Host port: {} Project: {}, Chosen DB: {}",
        zone.as_str(),
        host_port,
        project,
        chosen_db
